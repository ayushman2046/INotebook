...
const Notes = (props) => {
  ...
  const handleClick = (e) => {
    ...
    props.setalert('succesfully updated','success')
  }
  ...
  return (
    <>
      ...
      <div className="row my-3">
        <h2>Your Notes</h2>
        {notes.length===0 ? <div className="container mx-2">No notes to display</div> : notes.map((note) => {
          return <Noteitem key={note._id} setalert={props.setalert} updatenote={updatenote} note={note} />
        })}
      </div>
    </>
  )
